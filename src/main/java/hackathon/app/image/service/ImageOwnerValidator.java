    ...
    public void validate(ImageOwnerType type, String ownerId, Long userId) {
        ...
        try {
            exists = switch (type) {
                ...
                case SCHEDULE_ITEM -> count("SELECT COUNT(*) FROM schedule_items i JOIN schedules s ON s.id = i.schedule_id WHERE i.id = ?1 AND s.user_id = ?2 AND i.deleted_at IS NULL", ownerId, userId) > 0;
